...
export default function VibesProInput({ value = '', onChange, onSend, placeholder }: VibesProInputProps) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10 }}>
      ...
      <button type="button" onClick={onSend} style={{ padding: '10px 14px', borderRadius: 999, border: '1px solid #d4af37', background: '#f3d07b', color: '#111827', fontWeight: 700 }}>{t('vibespro.chat.send')}</button>
    </div>
  );
}
