...
const i18nText = fs.readFileSync(path.join(root, 'src/lib/i18n.ts'), 'utf8');
const start = i18nText.indexOf('export const translations');
const end = i18nText.indexOf('export function normalizeLanguage');
if (start === -1 || end === -1) {
...
if (missingInStreet.length) console.log('missing in street keys:', missingInStreet.slice(0,100).join(', '));
if (missingInBasic.length) console.log('missing in basic keys:', missingInBasic.slice(0,100).join(', '));
